...
export default function DeliveryPage() {
  ...
  const updateURL = (filters: typeof currentFilters) => {
    ...
    router.push(`/delivery?${params.toString()}`, { scroll: false });
  };
  ...
  const loadCampaigns = async (filters: typeof currentFilters, pageNum: number = 1, append: boolean = false) => {
    try {
      ...
      if (filters.region !== 'all') queryFilters.region = filters.region;
      ...
      setError(err instanceof Error ? err.message : '캠페인을 불러오는데 실패했습니다.');
      setLoading(false);
    }
  };
    ...
    <ErrorBoundary>
      <div className="w-full px-4 py-8">
        <div className="max-w-[1920px] mx-auto">
          <h1 className="text-2xl font-bold mb-6">배송 체험</h1>
